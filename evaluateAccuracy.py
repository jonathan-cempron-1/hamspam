...
# evaluate old spam emails
oldttotal = 33
oldspamclassified = 0
oldhamclassified = 0
for i in range(1,oldttotal):
	if isSpam("oldspamemails/mail%d.txt"%(i)):
		oldspamclassified += 1
	else:
		oldhamclassified += 1

# evaluate spam emails
sttotal = 747
sspamclassified = 0
shamclassified = 0
for i in range(1,sttotal):
	if isSpam("spamemails/email%d.txt"%(i)):
		sspamclassified += 1
	else:
		shamclassified += 1
...
hspamclassified = 0
hhamclassified = 0
for i in range(1,httotal):
	if isSpam("hamemails/email%d.txt"%(i)):
		hspamclassified += 1
	else:
		hhamclassified += 1
		
opc = float(oldspamclassified)/float(oldttotal)
spc = float(sspamclassified)/float(sttotal)
hpc = float(hhamclassified)/float(httotal)
tcorrect = oldspamclassified + sspamclassified + hhamclassified
tincorrect = oldhamclassified + shamclassified + hspamclassified
ttotal = oldttotal + sttotal + httotal
tpc = float(tcorrect)/float(ttotal)
print ""
print "SPAM DETECTOR EVALUATION"
print "                     correct\tincorrect\tsetTotal\tpercentCorrect"
print "old spam emails set :  %d\t%d\t\t%d\t\t%.2f"%(oldspamclassified, oldhamclassified, oldttotal, opc)
print "    spam emails set :  %d\t%d\t\t%d\t\t%.2f"%(sspamclassified, shamclassified, sttotal, spc)
print "     ham emails set :  %d\t%d\t\t%d\t\t%.2f"%(hhamclassified, hspamclassified, httotal, hpc)
print "          total set :  %d\t%d\t\t%d\t\t%.2f"%(tcorrect, tincorrect, ttotal, tpc)
print ""
